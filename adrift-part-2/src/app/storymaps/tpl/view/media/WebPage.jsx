...
export default class WebPage extends Media {
  ...
  buildScriptBlock() {
    ...
    // the content blocks
    const scriptId    = this._url.match(/id=['"][^'"]+/g)[0].split('="')[1];
    const scriptSrc   = this._url.match(/src=['"][^'"]+/g)[0].split('="')[1];
    const scriptTitle = this._url.match(/title=['"][^'"]+/g)[0].split('="')[1];
    return scriptBlock({
      scriptId,
      scriptSrc,
      scriptTitle,
  ...
  render(context) {
    ...
    if (this._isScript) {
      ...
      output += viewBlock({
        ...
        labelExploreStart: i18n.viewer.media.explore,
        labelExploreStop: i18n.viewer.media.exploreStop,
        style: 'max-height:' + this.getMaxHeight() + 'px;'
      });
    }
